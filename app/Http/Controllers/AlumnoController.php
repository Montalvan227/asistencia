...
class AlumnoController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $alumnos = Alumno::with('user')
            ->when($grados, function ($query) use ($grados) {
                $query->where('grado', $grados);
            })
            ->when($secciones, function ($query) use ($secciones) {
                $query->where('seccion', $secciones);
            })
            ->paginate(10);
        ...
        return view('dashboard.alumnos', compact('alumnos', 'grados', 'secciones'));
    }
    /**
